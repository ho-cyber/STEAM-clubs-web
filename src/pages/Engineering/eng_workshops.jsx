...
const EngWorkshops = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'engineering_workshops'), (querySnapshot) => {
      const workshopList = [];
  ...
  const handleSubmit = async (e) => {
    ...
    if (newWorkshop.name && newWorkshop.description && newWorkshop.link) {
      try {
        await addDoc(collection(db, 'engineering_workshops'), newWorkshop);
        setShowForm(false);
  ...
  const handleDelete = async (workshopId) => {
    try {
      await deleteDoc(doc(db, 'engineering_workshops', workshopId));
    } catch (error) {
  ...
  return (
    <section className='max-container'>
      ...
      <p className='text-slate-500 mt-2 leading-relaxed'>
        Explore Engineering workshops that we have planned.
      </p>
            ...
            <div className='lg:w-[400px] w-full' key={workshop.id}>
              ...
              <div className='mt-5 flex flex-col'>
                ...
                <div className='mt-5 flex items-center gap-2 font-poppins'>
                  {/* Link handling without condition */}
                  <a
                    href={workshop.link}
                    target='_blank'
