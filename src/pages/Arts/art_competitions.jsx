...
const ArtCompetitions = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'art_competitions'), (querySnapshot) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (newCompetition.name && newCompetition.description && newCompetition.link) {
      try {
        await addDoc(collection(db, 'art_competitions'), newCompetition);
        setShowForm(false);
  ...
  const handleDelete = async (competitionId) => {
    try {
      await deleteDoc(doc(db, 'art_competitions', competitionId));
    } catch (error) {
  ...
  return (
    <section className='max-container'>
      ...
      <p className='text-slate-500 mt-2 leading-relaxed'>
        Explore Art competitions that we have planned.
      </p>
...
export default ArtCompetitions;
