...
const SciWorkshops = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'arts_workshops'), (querySnapshot) => {
      const workshopList = [];
  ...
  const handleSubmit = async (e) => {
    ...
    if (newWorkshop.name && newWorkshop.description && newWorkshop.link) {
      try {
        await addDoc(collection(db, 'arts_workshops'), newWorkshop);
        setShowForm(false);
  ...
  const handleDelete = async (workshopId) => {
    try {
      await deleteDoc(doc(db, 'arts_workshops', workshopId));
    } catch (error) {
  ...
  return (
    <section className='max-container'>
      ...
      <p className='text-slate-500 mt-2 leading-relaxed'>
        Explore arts workshops that we have planned.
      </p>
