...
const SciWorkshops = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'science_workshops'), (querySnapshot) => {
      const workshopList = [];
  ...
  const handleSubmit = async (e) => {
    ...
    if (newWorkshop.name && newWorkshop.description && newWorkshop.link) {
      try {
        await addDoc(collection(db, 'science_workshops'), newWorkshop);
        setShowForm(false);
  ...
  const handleDelete = async (workshopId) => {
    try {
      await deleteDoc(doc(db, 'science_workshops', workshopId));
    } catch (error) {
  ...
  return (
    <section className='max-container'>
      ...
      <p className='text-slate-500 mt-2 leading-relaxed'>
        Explore science workshops that we have planned.
      </p>
