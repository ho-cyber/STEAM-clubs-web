...
const MathProjects = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'math_projects'), (querySnapshot) => {
      const projectList = [];
  ...
  const handleSubmit = async (e) => {
    ...
    if (newProject.name && newProject.description && newProject.link) {
      try {
        await addDoc(collection(db, 'math_projects'), newProject);
        setShowForm(false);
  ...
  const handleDelete = async (projectId) => {
    try {
      await deleteDoc(doc(db, 'math_projects', projectId));
    } catch (error) {
      ...
      <div className='flex items-center justify-between'>
        ...
        <h1 className='head-text'>
          <span className='blue-gradient_text drop-shadow font-semibold'>
            Maths projects that we have planned
          </span>
        </h1>
      </div>
